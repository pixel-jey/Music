...
public class Baidu {
	...
	private static String parseMusic(InputStream inputStream) {
		String durl = null;
		try {
			// 使用dom4j解析xml字符串
	...
	private static String searchMusic(String musicTitle, String musicAuthor) {
		// 百度音乐搜索地址
		...
		// 从返回结果中解析出Music
		String music = parseMusic(inputStream);
		return music;
	}
	...
	public static boolean search(String title, String artist) {
		...
		String mp3SRC = Baidu.searchMusic(title, artist);

		if (mp3SRC != null) {
			SearchInfo.down(mp3SRC, SearchInfo.MP3, title + ".mp3");
			flag = true;
		}
		return flag;
	}
